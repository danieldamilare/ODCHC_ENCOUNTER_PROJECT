...
for f in facilities:
    random_facilities.append((f, random.choice(LOCAL_GOVERNMENT),
                              random.choice(
                                  ['primary', 'private', 'secondary']),
                              ))

print("Inserting into facility")
cur.executemany(
    "INSERT INTO facility (name, local_government, facility_type) VALUES (?, ?, ?)", random_facilities)
...
for i in range(1, len(facilities) + 1):
    users.append(
        (f'user{i}', generate_password_hash(f'password{i}'), i,  'user'))


print("Inserting into users")

cur.executemany(
    "INSERT INTO users (username, password_hash, facility_id, role) VALUES (?, ?, ?, ?)", users)
...
disease_ids = [r['id']
               for r in cur.execute("SELECT id FROM diseases").fetchall()]
user_ids = [r['id'] for r in cur.execute('SELECT id FROM users')]
outcome_ids = [r['id']
               for r in cur.execute('SELECT id from treatment_outcome')]
...
encounter_ids = [r[0]
                 for r in cur.execute("SELECT id FROM encounters").fetchall()]
