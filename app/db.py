...
def seed_db():
    from app.services import UserServices, DiseaseServices, DiseaseCategoryServices
    ...
    click.echo(
        f"Added {len(category_list)} disease categories to the database")
    for x in disease_list:
        # random disease cateogry for testing
        DiseaseServices.create_disease(
            x, random.randint(1, len(category_list)), commit=False)
    ...
    click.echo(
        f"Created user username: {u.username}, facility_id: {u.facility_id}, role: {u.role.name}")
    db.commit()
