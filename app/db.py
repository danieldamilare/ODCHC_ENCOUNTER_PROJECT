...
def seed_db():
    ...
    import random
    f = FacilityServices.create_facility('SSH Okitipupa', 'okitipupa', 'tertiary')
    click.echo(f"Added facility id: {f.id}, name: {f.name}, local_government = {f.local_government}, type: {f.facility_type}")
    u = UserServices.create_user('betatester', 1, 'betatesterpassword')
    click.echo(f"Created user username: {u.username}, facility_id: {u.facility_id}, role: {u.role.name}")
    u = UserServices.create_user('betaadmintester', None, 'betaadmintesterpassword', role = Role.admin)
    click.echo(f"Created user username: {u.username}, facility_id: {u.facility_id}, role: {u.role.name}")
